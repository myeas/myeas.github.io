...
-  重新封装客户端带知识库   
   ...
   [金蝶EAS客户端含本地知识库](https://eas.sinomach-auto.cn/easWebClient/download/easclient_setup_eashelp.exe)
   
-  重新封装客户端不带知识库

   [金蝶EAS客户端_不含知识库](https://eas.sinomach-auto.cn/easWebClient/download/easclient_setup_noeashelp.exe)

::: warning 

因为封装的客户端，没有签名证书，可能会被浏览器误识别会不安全的文件，因此下载时，可能会提示`通常不会下载XXXX.exe。请在打开前确保信任XXXX.exe。`时，请务必点击旁边的三个点，点击`保留`，再点`显示详细信息`，再点击`仍然保留`，才可以把安装文件下载到本地。
